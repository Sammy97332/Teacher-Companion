...
def generate_report_card(student_id):
    ...
    for subject in subjects:
        ...
        # CA score: average percentage across all quizzes/tests/exercises
        if ca_assessments:
            ca_percentage = sum(a.score / a.max_score * 100 for a in ca_assessments) / len(ca_assessments)
        else:
            ca_percentage = None

        # Exam score: use the most recent exam recorded for the term
        if exam_assessments:
        ...
        # Only compute a final grade if we have both pieces
        if ca_percentage is not None and exam_percentage is not None:
            ...
            grade = _grade_for(final_percentage)
    ...
    return jsonify({
        ...
        "overall_grade": _grade_for(overall_average) if overall_average is not None else None,
    }), 200
